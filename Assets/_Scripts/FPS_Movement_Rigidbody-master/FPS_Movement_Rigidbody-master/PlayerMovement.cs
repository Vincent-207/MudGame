...
public class PlayerMovement : MonoBehaviour {
    ...
    void OnEnable()
    {
        crouch.action.canceled += StopCrouch;
        crouch.action.started += StartCrouch;
    }
    ...
    void StartCrouch(InputAction.CallbackContext callbackContext)
    {
        StartCrouch();
    }
    ...
    private void MyInput() {
        ...
        y = move.action.ReadValue<Vector2>().y;
        jumping = jump.action.ReadValue<float>() > 0.5f;
        // jumping = true;
        crouching = crouch.action.ReadValue<float>() > 0.5f;
    ...
    private void Look() {
        float mouseX = look.action.ReadValue<Vector2>().x * sensitivity * Time.fixedDeltaTime * sensMultiplier;
        float mouseY = look.action.ReadValue<Vector2>().y * sensitivity * Time.fixedDeltaTime * sensMultiplier;
